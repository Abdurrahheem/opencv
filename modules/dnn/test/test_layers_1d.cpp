...
typedef testing::TestWithParam<tuple<std::vector<int>, std::string>> Layer_ScatterND_Test;
TEST_P(Layer_ScatterND_Test, Accuracy1D) {
    ...
    lp.set("axis", 0);
    lp.set("reduction", opr);
    Ptr<Layer> layer = ScatterNDLayer::create(lp);
...
INSTANTIATE_TEST_CASE_P(/*nothing*/, Layer_ScatterND_Test, Combine(
/*input blob shape*/    testing::Values(std::vector<int>{4},
                                        std::vector<int>{1, 4}),
/*reduce*/              Values("none", "add", "mul", "max", "min")
));
